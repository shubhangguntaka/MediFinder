...
const MedicineListItem: React.FC<{item: Medicine}> = ({item}) => (
    <div className="flex justify-between items-start py-3 px-4 bg-slate-50/70 rounded-lg dark:bg-slate-700/50">
        <div className="min-w-0 flex-1 pr-4">
            <p className="font-semibold text-gray-800 dark:text-gray-200 break-words">{item.name}</p>
            {item.brands.length > 0 && (
                <div className="mt-1.5 flex flex-wrap gap-2">
                    {item.brands.map(brand => (
                        <span key={brand} className="text-xs font-medium bg-gray-200 text-gray-700 px-2 py-1 rounded-full dark:bg-slate-600 dark:text-slate-200 break-all">{brand}</span>
                    ))}
                </div>
            )}
        </div>
...
const ResultsList: React.FC<ResultsListProps> = ({ results, isLoading, error, searched, isGuest, onLoginClick, sortOption, onSortChange, showInStockOnly, onFilterChange }) => {
          ...
          <div className="bg-gradient-to-br from-primary-500 to-primary-600 p-6 rounded-xl shadow-lg mb-8 text-white">
            <div className="flex items-start gap-4 mb-4">
              ...
              <div className="min-w-0">
                <p className="text-sm font-semibold text-primary-100 uppercase tracking-wide">About {medicineName}</p>
                <h3 className="font-bold text-2xl capitalize break-words">{medicineName}</h3>
              </div>
            </div>
            <p className="text-primary-50 leading-relaxed text-md mb-4 break-words">{medicineInfo.description}</p>
             <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-4 text-sm border-t border-white/20 pt-4">
                <div className="min-w-0">
                    <p className="font-semibold text-primary-100 uppercase tracking-tighter">Primary Use</p>
                    <p className="text-primary-50 mt-1 break-words">{medicineInfo.primaryUse}</p>
                </div>
                <div className="min-w-0">
                    <p className="font-semibold text-primary-100 uppercase tracking-tighter">Common Forms</p>
                    <p className="text-primary-50 mt-1 break-words">{medicineInfo.commonForms}</p>
                </div>
            </div>
          </div>
        )}
        {!isGuest && (
            <div className="flex flex-col sm:flex-row gap-4 justify-between items-center mb-6 p-4 bg-white dark:bg-slate-800 rounded-lg shadow-sm border dark:border-slate-700">
                <div className="flex items-center gap-2 w-full sm:w-auto">
                    ...
                    <select id="sort-by" value={sortOption} onChange={(e) => onSortChange(e.target.value as 'distance' | 'stock')} className="bg-slate-100 dark:bg-slate-700 border-slate-200 dark:border-slate-600 rounded-md text-sm focus:ring-primary-500 focus:border-primary-500 w-full sm:w-auto">
                        <option value="distance">Distance (nearest)</option>
                        <option value="stock">Stock (highest)</option>
                    </select>
                </div>
                <div className="relative flex items-start self-start sm:self-auto">
        ...
        <div className="grid gap-6 grid-cols-1 md:grid-cols-2 lg:grid-cols-3">
          {displayData.length > 0 ? displayData.map((pharmacy, index) => (
            <ResultCard key={`${pharmacy.name}-${index}`} pharmacy={pharmacy} isGuest={isGuest} onLoginClick={onLoginClick} />
             ...
             <div className="md:col-span-2 lg:col-span-3 text-center py-10 px-4 bg-white dark:bg-slate-800 rounded-xl shadow-sm border dark:border-slate-700">
                <SearchIcon className="w-16 h-16 mx-auto text-gray-400 dark:text-slate-600 opacity-75 mb-4" />
                ...
                <p className="mt-2 text-gray-500 dark:text-gray-400">
                    {data.length > 0 
                        ...
                        : "We couldn't find any pharmacies with this medicine nearby."
                    }
                </p>
            </div>
          )}
  ...
  if (results?.type === 'store') {
      ...
      return (
          <div className="bg-white p-6 sm:p-8 rounded-xl shadow-sm border border-gray-200/80 max-w-3xl mx-auto dark:bg-slate-800 dark:border-slate-700 overflow-hidden">
              <div className="flex items-start gap-4 mb-4">
                  ...
                  <div className="min-w-0">
                      <h3 className="font-bold text-2xl text-gray-900 dark:text-gray-100 break-words leading-tight">{store.storeName}</h3>
                      <p className="text-md text-gray-600 dark:text-gray-400 break-words mt-1">{store.address}</p>
                      {distance !== -1 && <p className="text-sm text-gray-500 dark:text-gray-400 mt-2 font-medium">{distance.toFixed(1)} km away</p>}
                  </div>
              </div>
              <h4 className="font-semibold text-lg text-gray-800 dark:text-gray-200 mt-6 mb-3 border-t dark:border-slate-700 pt-6">Available Inventory</h4>
              <div className="space-y-3">
                {inventory.length > 0 ? inventory.map(item => <MedicineListItem key={item.name} item={item}/>) : <p className="text-sm text-gray-500 dark:text-gray-400 text-center py-6 bg-slate-50 dark:bg-slate-900/50 rounded-lg italic">This pharmacy has not listed any inventory yet.</p>}
              </div>
          </div>
      );
  }
