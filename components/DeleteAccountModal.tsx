...
const DeleteAccountModal: React.FC<DeleteAccountModalProps> = ({ onClose }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!result.success) {
        // Error toast is shown from AuthContext
        setIsProcessing(false);
    }
    // On success, AuthProvider logs the user out and shows a toast.
    // This component will unmount, so no need to call onClose().
  };
